...
client.on("guildMemberAdd", async (member) => {
  console.log(`[v0] Member joined: ${member.user.tag} (ID: ${member.user.id}) in guild ${member.guild.id}`);
  console.log(`[v0] Checking if guild ID ${member.guild.id} === CAELUM_GUILD_ID ${CAELUM_GUILD_ID}`);
  
  // Caelum Server: Send verification embed in DM
  if (member.guild.id === CAELUM_GUILD_ID) {
    console.log(`[v0] Member is in Caelum server!`);
    
    // Skip bots
    if (member.user.bot) {
      console.log(`[v0] Skipped bot user ${member.user.tag}`);
    } else {
      try {
        ...
        console.log(`[v0] Attempting to send DM to ${member.user.tag}`);
        await member.send({ embeds: [verifyEmbed], components: [verifyRow] });
        console.log(`[v0] Successfully sent verification embed to ${member.user.tag}`);
      } catch (err) {
        console.error(`[v0] Failed to send verification embed to ${member.user.tag}:`, err.message);
        console.error(`[v0] Full error:`, err.code, err.status);
      }
    }
  }
...
\`\`\``;
  ...
  if (content.startsWith(`${PREFIX}changebeam`)) {
    try {
      ...
      if (!args) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Please provide a hex color code. Usage: `!changebeam #FF5733`",
        });
        return;
      }
      ...
      if (!hexColorRegex.test(args)) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Invalid hex color code. Please use format: `#FF5733` or `FF5733`",
        });
        return;
      }

      // Ensure hex code starts with #
      let colorCode = args.startsWith("#") ? args : `#${args}`;

      // Find the "beam" role (case-insensitive search for roles containing "beam")
      const beamRole = message.guild.roles.cache.find(
        (role) => role.name.toLowerCase().includes("beam")
      );

      if (!beamRole) {
        await message.reply({
          content: "<:emoji_11:1506864561435967509> Could not find a role with 'beam' in its name.",
        });
        return;
      }
      ...
      // Check if the role is higher than the bot's highest role
      if (beamRole.position >= message.guild.members.me.roles.highest.position) {
        await message.reply({
      ...
      await beamRole.setColor(colorCode);

      await message.reply({
        content: `<:emoji_14:1508646444607864872> Successfully changed the ${beamRole.name} role color to ${colorCode}!`,
      });

      console.log(`[v0] ${message.author.username} changed ${beamRole.name} color to ${colorCode}`);
    } catch (err) {
